...
namespace EnvMgr
{
    public partial class RestoreDB : Form
    {
        ...
        public void RestoreDatabase(string[] selectedFiles, string filePath, string sqlServ, string sqlUser, string sqlPassword, string backupName)
        {
        ...
        {
            RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\Environment Manager");
            string filePath = Convert.ToString(key.GetValue("DB Folder")) + Form1.selectedGPVersion + "\\";
            ...
            Thread executeRestore = new Thread(() => RestoreDatabase(listOfSelectedFiles.ToArray(), filePath, sqlServ, sqlUser, sqlPassword, backupName));
